...
void __JAP_PG_RETRIEVE_FC(JAP_LAYER* LAYER,  JAP_INTER_IDX* itr, JAP_INTRA_IDX* ita, JAP_TILE_SIZE* ts, int* l_cnt);

void __HAW_FETCH_INPUT_CONV(JAP_DATA *D_IN, _q15* DST , JAP_INTER_IDX* itr, JAP_INTRA_IDX* ita, JAP_TILE_SIZE* ts);
void __HAW_FETCH_WEIGHT(JAP_PARA *PARA , JAP_INTER_IDX* itr, JAP_INTRA_IDX* ita, JAP_TILE_SIZE* ts);
void __HAW_mpypara_init(msp_matrix_mpy_q15_params* mpyParams, JAP_TILE_SIZE* ts, uint16_t batch);
msp_status __HAW_matrix_mpy_q15(const msp_matrix_mpy_q15_params *params, const _q15 *srcA, const _q15 *srcB, _q15 *dst, uint32_t destNV, uint32_t FP_addr, uint32_t* FP_val , uint16_t BATCH);
void __HAW_ADD(const msp_add_q15_params *params, const _q15 *srcA, const _q15 *srcB, _q15 *dst ,uint32_t destNV, uint32_t FP_addr, uint32_t* FP_val , uint16_t BATCH);
void __HAW_PG_RETRIEVE_FC(JAP_LAYER* LAYER,  JAP_INTER_IDX* itr, JAP_INTRA_IDX* ita, JAP_TILE_SIZE* ts, uint32_t* fp);
...
void __JAP_PG_RETRIEVE_FC(JAP_LAYER* LAYER,  JAP_INTER_IDX* itr, JAP_INTRA_IDX* ita, JAP_TILE_SIZE* ts, int* l_cnt){
	...
	uint32_t b_offset = Aoffset3D( 1 , 1 , MCH_F ,1, MCH_F)*sizeof(_q15);
		...
		uint16_t tmp=1;
			...
			for(itr->m=0; itr->m <MCH  ; itr->m +=Tm){for(itr->n=0;  itr->n <NCH  ; itr->n +=Tn){
					...
					if(tmp == *l_cnt){
						...
						ADDR.L = PB + Aoffset3D(0,0, (ts->tm + NFPs) - 1 , 1 , ts->tm + NFPs ) * sizeof(_q15);
						if( ts->tm < batch){ //align
...
//							*ita = (JAP_INTRA_IDX ){.r=0, .c=0, .m=0, .n=0, .op=1, .flip=ita->flip};
							...
							for(ita->m = 0 ; ita->m < ts->tm ; ita->m+=batch ){
								ADDR.L = (ita->flip^0x1)*b_offset + D_OUT_Ptr + Aoffset3D( 0, 0 , (itr->m+ ita->m)+ batch + ((itr->m+ ita->m)/batch),1, MCH_F)*sizeof(_q15);
								SPI_READ(&ADDR,(uint8_t*)&pb_L,sizeof(_q15));
...
void JAP_FC(JAP_LAYER* LAYER){
	...
	uint16_t trans=0;
	uint16_t MCH_F =  MCH+(MCH + batch-1)/batch;
	uint32_t b_offset = MCH_F*sizeof(_q15);
	...
	//progress search
	__JAP_PG_RETRIEVE_FC(LAYER,  &itr, &ita, &ts, &loop_cnt);
	for(;itr.m < ts.tm;itr.m++){
		for(;itr.n < ts.tn;itr.n++){
			ts.tm = (itr.m+Tm > MCH) ? (MCH  - itr.m) : Tm;
			ts.tn = (itr.n+Tn > NCH) ? (NCH  - itr.n) : Tn;
			...
			for(; ita.op<2 ;ita.op++){
				if(ita.op == 0){
					...
					prog_oft =  Aoffset3D(0 ,0 , ita.m + ita.m/batch,1, (ts.tm + NFPs) + ((ts.tm + NFPs)&0x01) );
					srcA += prog_oft;
					if( (itr.n==0) ){
						ADDR.L = D_OUT_Ptr + Aoffset3D( 0, 0 , (itr.m+ ita.m) + ((itr.m+ ita.m)/batch),1, MCH_F)*sizeof(_q15);
						...
						ADDR.L = (ita.flip)*b_offset + D_OUT_Ptr + Aoffset3D( 0, 0, (itr.m+ ita.m) + ((itr.m+ ita.m)/batch),1, MCH_F)*sizeof(_q15);
...
void __HAW_PG_RETRIEVE_FC(JAP_LAYER* LAYER,  JAP_INTER_IDX* itr, JAP_INTRA_IDX* ita, JAP_TILE_SIZE* ts, uint32_t* fp){
	...
	uint16_t lea_o_offset = lea_w_offset + (Tn+2) * Tm*2;
		lea_o_offset += (lea_o_offset &0x1);
	ADDR.L = LAYER->FOOTPRINT;
	SPI_READ(&ADDR,(uint8_t*)&fp,sizeof(_q15));
	...
	for(;itr->m < ts->tm;itr->m++){
		for(;itr->n < ts->tn;itr->n++){
			...
			for(; ita->op<2 ;ita->op++){
				if(ita->op == 0){
					if(loop_cnt >= *fp){*fp = loop_cnt;  return;}
					else{loop_cnt+=  batch;}
				}else{
					for(ita->m=0;ita->m < ts->tm;ita->m+=batch){
						if(loop_cnt >= *fp){*fp = loop_cnt;  return;}
...
msp_status __HAW_matrix_mpy_q15(const msp_matrix_mpy_q15_params *params, const _q15 *srcA, const _q15 *srcB, _q15 *dst, uint32_t destNV, uint32_t FP_addr, uint32_t* FP_val , uint16_t BATCH)
{
    ...
    uint16_t _batch;
    uint32_t _fp = FP_val;
//    uint16_t FP_val = FP_val_i;
    msp_status status;
    MSP_LEA_MPYMATRIXROW_PARAMS *leaParams;
    _q15 *dstTX = dst;

    _batch = BATCH > srcBCols ? srcBCols : BATCH;
    ...
    while (srcARows--) {
        ...
        for(int i =0;i<srcBCols;i+=BATCH){
        	ADDR.L = destNV;
        	SPI_WRITE(&ADDR,(uint8_t*)dstTX,sizeof(_q15)*_batch);
        	dstTX+=_batch;
        	destNV+=_batch*sizeof(_q15);
        	(*FP_val)+=_batch;
        	ADDR.L = FP_addr;
        	SPI_WRITE(&ADDR,(uint8_t*)FP_val,sizeof(uint32_t));
...
void __HAW_ADD(const msp_add_q15_params *params, const _q15 *srcA, const _q15 *srcB, _q15 *dst ,uint32_t destNV, uint32_t FP_addr, uint32_t* FP_val , uint16_t BATCH)
{
    ...
    length = params->length;
    if (length==0)return ;
    _batch = BATCH > length ? length : BATCH;
...
void __HAW_FETCH_WEIGHT(JAP_PARA *PARA , JAP_INTER_IDX* itr, JAP_INTRA_IDX* ita, JAP_TILE_SIZE* ts){
	...
	if( (ts->tn & 0x1) == 0 ){
		for(uint16_t i=0;i < ( ts->tm );i++)srcW[i]=0;
	}
}
...
void __HAW_mpypara_init(msp_matrix_mpy_q15_params* mpyParams, JAP_TILE_SIZE* ts, uint16_t batch){
	...
	if(ts->tn & 0x1){
		...
		mpyParams->srcACols = ts->tn;
		mpyParams->srcBRows = ts->tn;
	}
}


void HAW_FC(JAP_LAYER* LAYER){
	...
	uint32_t b_offset = MCH*sizeof(_q15);
	...
	JAP_TILE_SIZE ts  = {.tr=Tr, .tc=Tc, .tm=Tm, .tn=Tn};
	//progress search
	__HAW_PG_RETRIEVE_FC(LAYER,  &itr, &ita, &ts, &footprint);
	for(;itr.m < ts.tm;itr.m++){
		for(;itr.n < ts.tn;itr.n++){
			ts.tm = (itr.m+Tm > MCH) ? (MCH  - itr.m) : Tm;
			ts.tn = (itr.n+Tn > NCH) ? (NCH  - itr.n) : Tn;

			__HAW_mpypara_init(&mpyParams, &ts,batch);
			for(; ita.op<2 ;ita.op++){
				if(ita.op == 0){
					__HAW_FETCH_WEIGHT(PARA,&itr,&ita,&ts);
					srcC32 = PB ;
					if( ts.tm < batch){
						srcC32 +=  ((batch - ts.tm)*sizeof(_q15));
					}
					__HAW_FETCH_INPUT_CONV(D_IN, LEA_MEMORY , &itr, &ita, &ts);
					__HAW_matrix_mpy_q15(&mpyParams,LEA_MEMORY,srcA,srcB, srcC32, LAYER->FOOTPRINT, &footprint , batch);
					...
					prog_oft =  Aoffset3D(0 ,0 , ita.m ,1, ts.tm  );
					srcA += prog_oft;
					if( (itr.n==0) ){
						ADDR.L = D_OUT_Ptr + Aoffset3D( 0, 0 , (itr.m+ ita.m) + ((itr.m+ ita.m)/batch),1, MCH)*sizeof(_q15);
						trans = ts.tm - ita.m ;
						SPI_WRITE(&ADDR,(uint8_t*)srcA,trans*sizeof(_q15));
						footprint+= trans;
						ADDR.L = LAYER->FOOTPRINT;
						SPI_WRITE(&ADDR, (uint8_t*)&footprint, sizeof(uint32_t));
					}else{
						ADDR.L = D_OUT_Ptr + Aoffset3D( 0, 0 , (itr.m+ ita.m) + ((itr.m+ ita.m)/batch),1, MCH)*sizeof(_q15);
						trans = ts.tm + NFPs - (ita.m + ita.m/batch );
						SPI_READ(&ADDR,(uint8_t*)LEA_MEMORY,trans*sizeof(_q15));
						addParams.length = trans + (trans&0x1);
						ADDR.L = (ita.flip)*b_offset + D_OUT_Ptr + Aoffset3D( 0, 0, (itr.m+ ita.m),1, MCH)*sizeof(_q15);

						__HAW_ADD(&addParams, srcA, LEA_MEMORY,LEA_MEMORY ,ADDR.L, LAYER->FOOTPRINT, &footprint , batch);
//						srcA += (trans  + ((ts.tm + NFPs)&0x01) ) ;
					}
					ita.m=0;
				}
